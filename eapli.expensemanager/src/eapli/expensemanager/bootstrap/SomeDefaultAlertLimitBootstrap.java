...
public class SomeDefaultAlertLimitBootstrap implements Bootstrap {
	...
	private void ensureLimitWeekExpenditureExists(
			final AlertLimitType alertLimitType) {
		if (AlertLimitExpenditure.findByAlertType(alertLimitType) == null) {
			AlertLimitExpenditure alertLimitWeekExpenditure = new AlertLimitExpenditure(
					alertLimitType, LIMIT_WEEK_YELLOW, LIMIT_WEEK_RED);
	...
	private void ensureLimitMonthExpenditureExists(AlertLimitType alertLimitType) {
		if (AlertLimitExpenditure.findByAlertType(alertLimitType) == null) {
			AlertLimitExpenditure alertLimitMonthExpenditure = new AlertLimitExpenditure(
					alertLimitType, LIMIT_MONTH_YELLOW, LIMIT_MONTH_RED);
	...
	private void ensureLimitDeviationExists(final AlertLimitType alertLimitType) {
		...
		for (ExpenseType eT : expenseTypes) {
			AlertLimitByExpenseType alertLimitET = new AlertLimitByExpenseType(
					alertLimitType, LIMIT_DEVIATION_YELLOW,
	...
	private void ensureLimitMinimumBalanceExists(
		...
		if (AlertLimitExpenditure.findByAlertType(alertLimitType) == null) {
			AlertLimitExpenditure alertLimitMonthExpenditure = new AlertLimitExpenditure(
					alertLimitType, LIMIT_BALANCE_YELLOW, LIMIT_BALANCE_RED);
			alertLimitMonthExpenditure.save();
		}
	}
}
