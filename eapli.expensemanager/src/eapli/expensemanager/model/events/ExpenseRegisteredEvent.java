...
public class ExpenseRegisteredEvent {
    ...
    public ExpenditureOverLimitAlertEvent IsOverLimitWeekExpenditure() {
        CheckingAccountRepository repo =
                PersistenceFactory.buildPersistenceFactory().
    ...
    public ExpenditureOverLimitAlertEvent buildAlertEvent(BigDecimal expenditure,
        ...
        if (alertLimit != null) {
            if (expenditure.compareTo(alertLimit.getLimitRed()) > 0) {
                return new ExpenditureOverLimitAlertEvent(alertLimit.getAlertType().
                        ...
                        getLimitRed(), expenditure, "RED");
            }
            if (expenditure.compareTo(alertLimit.getLimitYellow()) > 0) {
                return new ExpenditureOverLimitAlertEvent(alertLimit.getAlertType().
                        getDescription(),
                                                 alertLimit.getLimitYellow(), alertLimit.
                        getLimitRed(), expenditure, "YELLOW");
            }
            return null;
        }
        return null;
    ...
    private ExpenditureByExpenseTypeOverLimitAlertEvent buildAlertEventDeviation(BigDecimal amount,
                                                             BigDecimal average,
        ...
        if ((amount.compareTo(limMinRed) <= 0) || (amount.compareTo(limMaxRed) >= 0)) {
            return new ExpenditureByExpenseTypeOverLimitAlertEvent(
                    alertLimitET.getAlertType().getDescription(),
                    yellowPercent, redPercent, amount, average,
                    "RED", alertLimitET.getExpenseType());
        }
        if ((amount.compareTo(limMinYellow) <= 0) || (amount.
                compareTo(limMaxYellow) >= 0)) {
            return new ExpenditureByExpenseTypeOverLimitAlertEvent(
                    alertLimitET.getAlertType().getDescription(),
                    yellowPercent, redPercent, amount, average,
                    "YELLOW", alertLimitET.getExpenseType());
        }
        return null;
    }
}
