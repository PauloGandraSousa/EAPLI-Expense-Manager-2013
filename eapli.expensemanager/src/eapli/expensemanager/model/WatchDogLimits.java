...
public class WatchDogLimits extends Observable implements Observer {
		...
		for (AlertLimitType alertLimType : types) {
			...
			case LIMIT_DEVIATION_BY_EXPENSE_TYPE:
				...
				if (alertLimitET != null) {
					AlertEventByExpenseType alertEventByET = buildAlertEventAverageDeviationByExpenseType(
							expense, alertLimitET);
			...
			case LIMIT_MINIMUM_BALANCE:
				...
				if (alertLimitBalance != null) {
					AlertEvent alert = buildAlertBalanceEvent(
							alertLimitBalance, expense);
					notifyObservers(alert);
				}
				break;
			}
	...
	private AlertEvent buildAlertEvent(AlertLimitExpenditure alertLimit,
		...
		case 1:
			this.setChanged();
			alert = new AlertEventExpenditure(alertLimit.getAlertType()
					.getDescription(), yellow, red, expenditure, "YELLOW");
			break;
		case 2:
			this.setChanged();
			alert = new AlertEventExpenditure(alertLimit.getAlertType()
					.getDescription(), yellow, red, expenditure, "RED");
			break;
		}
		return alert;
	}
	...
	private AlertEvent buildAlertBalanceEvent(AlertLimitExpenditure alertLimit,
		...
		case 1:
			this.setChanged();
			alert = new AlertEventExpenditure(alertLimit.getAlertType()
					.getDescription(), yellow, red, balance, "YELLOW");
			break;
		case 2:
			this.setChanged();
			alert = new AlertEventExpenditure(alertLimit.getAlertType()
					.getDescription(), yellow, red, balance, "RED");
			break;
		}
		return alert;
	}

	private AlertEventByExpenseType buildAlertEventAverageDeviationByExpenseType(
		...
		AlertEventByExpenseType alertEvent = buildAlertEventByExpenseType(
				alertLimitET, expenseRegisteredEvent.getAmount(), average, eT);
		return alertEvent;
	}

	private AlertEventByExpenseType buildAlertEventByExpenseType(
			AlertLimitByExpenseType alertLimitET, BigDecimal amount,
			BigDecimal average, ExpenseType eT) {
		...
		case 1:
			this.setChanged();
			alert = new AlertEventByExpenseType(alertLimitET.getAlertType()
					.getDescription(), yellow, red, amount, average, "YELLOW",
					eT);
			break;
		case 2:
			this.setChanged();
			alert = new AlertEventByExpenseType(alertLimitET.getAlertType()
					.getDescription(), yellow, red, amount, average, "RED", eT);
			break;
		}
		return alert;
	}

	private int compare(BigDecimal expenditure, BigDecimal yellow,
			BigDecimal red) {
	...
	private int compareBalance(BigDecimal balance, BigDecimal yellow,
			BigDecimal red) {
	...
	private int compareDeviation(BigDecimal valor, BigDecimal average,
		...
		if ((valor.compareTo(limMinRed) <= 0)
				|| (valor.compareTo(limMaxRed) >= 0)) {
			return 2;
		}
		if ((valor.compareTo(limMinYellow) <= 0)
				|| (valor.compareTo(limMaxYellow) >= 0)) {
			return 1;
		}
		return 0;
	}
